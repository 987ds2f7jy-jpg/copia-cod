...
| Serviço de planos | elegibilidade/ativação; email, nome, documento e refs | base URL/API key Edge, timeout (`supabase/functions/_shared/plans-service/client.ts:3-25`, `supabase/functions/_shared/plans-service/client.ts:199-324`); sem retry genérico | sem webhook local confirmado; coverage pode cair para self-pay; comportamento externo pendente |
