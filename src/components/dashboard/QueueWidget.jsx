...
function DetailItem({ label, value, multiline = false }) {
  return (
    <div className={`rounded-xl border border-gray-200 bg-gray-50/70 p-3 ${multiline ? 'col-span-2' : ''}`}>
      <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">{label}</p>
      <p className={`mt-1 text-sm text-gray-700 ${multiline ? 'whitespace-pre-wrap leading-6' : ''}`}>
  ...
  return (
    <>
            ...
            <div className="space-y-3 px-4 pb-4">
              {queuePatients.map((patient, index) => {
                ...
                return (
                  <div
                    key={patient.id}
                    className="rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50 px-4 py-4 shadow-sm"
                  >
                      ...
                      <div className="flex min-w-0 items-start gap-3">
                        ...
                        <div className="min-w-0 space-y-2">
                          <div className="space-y-1">
                            <p className="truncate text-sm font-semibold text-gray-900">{patient.patient_name}</p>
      ...
      <Sheet open={Boolean(selectedPatient)} onOpenChange={(open) => !open && setSelectedPatientId(null)}>
        <SheetContent
          side="right"
          className="w-full border-gray-200 bg-white p-0 text-gray-900 sm:max-w-2xl [&>button]:text-gray-400 [&>button]:hover:text-gray-700"
        >
          {selectedPatient ? (
            <div className="flex h-full flex-col">
              <SheetHeader className="border-b border-gray-100 px-6 py-5">
                ...
                <SheetTitle className="mt-2 text-xl text-gray-900">{selectedPatient.patient_name}</SheetTitle>
                ...
                <div className="space-y-5">
                  <section className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-900">Dados do paciente</h3>
                    </div>
                  ...
                  <section className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-900">Informacoes de saude</h3>
                    </div>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <section className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-900">Solicitacao do laudo</h3>
                    </div>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <section className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-900">Arquivos enviados</h3>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {selectedAttachments.map((fileUrl, index) => (
                          <a
                            ...
                            className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 text-sm text-sky-700 transition-colors hover:border-sky-200 hover:bg-sky-50"
                          >
