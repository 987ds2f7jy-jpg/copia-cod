    ...
    <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
      ...
      {reviews.length === 0 ? (
        <div className="py-10 text-center">
          ...
          <p className="text-muted-foreground text-sm">Ainda não há avaliações</p>
        </div>
      ) : (
        <>
          <div className="space-y-4">
            {visible.map(review => (
              <div key={review.id} className="p-4 rounded-xl bg-muted/50 border border-border">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="font-medium text-foreground text-sm">{anonymize(review.patient_name)}</p>
                    {review.created_date && (
