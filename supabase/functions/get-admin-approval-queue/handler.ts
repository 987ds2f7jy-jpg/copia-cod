...
import { createGetAdminApprovalQueueRuntime } from './repository.ts';
import { getAdminApprovalQueue } from './service.ts';
import { parseGetAdminApprovalQueueInput } from './validation.ts';

const FUNCTION_NAME = 'get-admin-approval-queue';
const CORS: CorsOptions = { allowedMethods: ['POST'] };

export async function handleGetAdminApprovalQueueRequest(req: Request) {
  ...
  try {
    ...
    const input = parseGetAdminApprovalQueueInput(body);
    const runtime = createGetAdminApprovalQueueRuntime();
