...
import { createGetFinanceDashboardRuntime } from './repository.ts';
import { getFinanceDashboard } from './service.ts';
import { parseGetFinanceDashboardInput } from './validation.ts';

const FUNCTION_NAME = 'get-finance-dashboard';
const CORS: CorsOptions = { allowedMethods: ['POST'] };

export async function handleGetFinanceDashboardRequest(req: Request) {
  ...
  try {
    ...
    const input = parseGetFinanceDashboardInput(body);
    const runtime = createGetFinanceDashboardRuntime();
