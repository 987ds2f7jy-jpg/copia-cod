...
import { createGetPaymentStatusRuntime } from './repository.ts';
import { getPaymentStatus } from './service.ts';
import { parseGetPaymentStatusInput } from './validation.ts';

const FUNCTION_NAME = 'get-payment-status';
const CORS: CorsOptions = { allowedMethods: ['POST'] };

export async function handleGetPaymentStatusRequest(req: Request) {
  ...
  try {
    ...
    const input = parseGetPaymentStatusInput(body);
    const runtime = createGetPaymentStatusRuntime();
