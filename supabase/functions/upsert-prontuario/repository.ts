...
function createUpsertProntuarioRepository(client: SupabaseClient): UpsertProntuarioRepository {
  return {
    ...
    async updateConsultationSession(params): Promise<ConsultationRow> {
      const { data, error } = await client
        .from('consultas')
        .update({
    ...
    async createProntuario(params): Promise<ProntuarioRow> {
      const { data, error } = await client
        .from('prontuarios')
        .insert({
          ...
          modo: params.mode,
          motivo_consulta: params.motivoConsulta,
          historico_risco: params.historicoRisco,
          exames_imagem: params.examesImagem,
          exame_fisico: params.exameFisico,
          avaliacao_diagnostico: params.avaliacaoDiagnostico,
    ...
    async updateProntuario(params): Promise<ProntuarioRow> {
      const { data, error } = await client
        .from('prontuarios')
        .update({
          modo: params.mode,
...
export function createUpsertProntuarioRuntime() {
  const client = createServiceRoleClient();

  return {
    authUserLookup: createSupabaseAuthUserLookup(client) as AuthenticatedUserLookup,
    repository: createUpsertProntuarioRepository(client),
  };
}
