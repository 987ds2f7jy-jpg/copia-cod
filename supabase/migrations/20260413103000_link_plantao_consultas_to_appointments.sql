...
SELECT
  c.paciente_id,
  coalesce(c.paciente_nome, ''),
  coalesce(c.paciente_email, ''),
  coalesce(c.profissional_id, ''),
  coalesce(c.profissional_nome, ''),
  coalesce(c.especialidade, ''),
  'IMEDIATO',
  coalesce(
    nullif(trim(coalesce(c.datetime, '')), ''),
    to_char(timezone('UTC', c.created_date), 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
  ),
  to_char(timezone('America/Sao_Paulo', c.created_date), 'YYYY-MM-DD'),
  to_char(timezone('America/Sao_Paulo', c.created_date), 'HH24:MI'),
  CASE
    WHEN lower(trim(coalesce(c.status, ''))) = 'finalizada' THEN 'completed'
    WHEN lower(trim(coalesce(c.status, ''))) = 'cancelada' THEN 'cancelled'
    WHEN lower(trim(coalesce(c.status, ''))) = 'em_atendimento' THEN 'in_progress'
    ELSE 'accepted'
  END,
  coalesce(c.preco, 0),
  coalesce(c.descricao_sintomas, ''),
  coalesce(
    nullif(trim(coalesce(c.datetime, '')), ''),
    to_char(timezone('UTC', c.created_date), 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
  ),
  c.id::TEXT
FROM public.consultas AS c
WHERE trim(coalesce(c.tipo_consulta, '')) = 'plantao'
  AND trim(coalesce(c.paciente_id, '')) <> ''
  AND trim(coalesce(c.profissional_id, '')) <> ''
  AND NOT EXISTS (
    SELECT 1
    FROM public.appointments AS a
    WHERE trim(coalesce(a.consulta_id, '')) = trim(c.id::TEXT)
  );
