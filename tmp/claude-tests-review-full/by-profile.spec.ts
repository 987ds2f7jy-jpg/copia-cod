...
rdTest.describe('by-profile — com paciente autenticado', () => {
  ...
  rdTest('sem disponibilidade configurada exibe aviso ao selecionar data @critical', async ({
    page, goto,
  }) => {
    rdTest.skip(!hasProfId, 'Define E2E_PROFESSIONAL_PUBLIC_ID.');
    rdTest.skip(
      !process.env.E2E_PROF_NO_AVAILABILITY,
      'Define E2E_PROF_NO_AVAILABILITY=true quando o profissional de teste não tiver disponibilidade configurada.',
    );
    ...
    const enabledDay = page.locator('table[role="grid"] button[name]')
      ...
      .first();
    if (await enabledDay.count() === 0) { rdTest.skip(true, 'Sem dias habilitados no calendário.'); return; }
    await enabledDay.click();

    // AgendamentoPerfil.jsx: "Profissional não configurou disponibilidade ainda."
    await expect(
      page.getByText('Profissional não configurou disponibilidade ainda.')
  ...
  rdTest('mensagem de slot ocupado é visível ao tentar horário já reservado @critical', async ({
    ...
    // Para testar: criar agendamento com E2E_SLOT_CONFLICT_DATETIME no banco,
    // depois tentar agendar o mesmo slot e verificar o erro.
    rdTest.skip(
      !process.env.E2E_SLOT_CONFLICT_DATETIME,
      'Define E2E_SLOT_CONFLICT_DATETIME com datetime de slot já ocupado (YYYY-MM-DDTHH:MM:00).',
    );
    // TODO: navegar até o slot específico e verificar mensagem de conflito.
    // Quando E2E_SLOT_CONFLICT_DATETIME estiver disponível, implementar aqui.
  });
  ...
  rdTest.skip(
    true,
    [
      'LIMITAÇÃO CONHECIDA — não testável com E2E de usuário único.',
      'A verificação de conflito em AgendamentoPerfil.jsx é client-side:',
      'dois pacientes simultâneos passam ambos pela verificação e criam agendamento duplo.',
      'Recomendação: mover para o Edge Function create-appointment com transação/lock.',
    ].join(' '),
  );
