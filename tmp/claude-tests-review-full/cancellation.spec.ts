...
rdTest.describe('cancellation — histórico e avaliação', () => {
  ...
  rdTest('aba histórico aceita status "CONCLUIDO" (PT) e "completed" (EN) — R4 @critical', async ({
    page, goto,
  }) => {
    rdTest.skip(
      !process.env.E2E_HAS_COMPLETED_APPOINTMENT,
    ...
    // getStatusBadge mapeia CONCLUIDO → "Concluída" e completed → "Concluída"
    // Pelo menos um badge deve aparecer
    await expect(
      page.getByText('Concluída').first()
    ).toBeVisible({ timeout: 8_000 });
  });
...
// ---------------------------------------------------------------------------
// R4 — consistência de labels de status (sem seed necessário)
// ---------------------------------------------------------------------------
rdTest.describe('cancellation — labels de status sem seed', () => {
  ...
  rdTest('getStatusBadge: aba próximas exibe label "Aguardando especialista" para SOLICITADO — R4', async ({
    page, goto,
  }) => {
    rdTest.skip(
      !process.env.E2E_HAS_SOLICITADO_APPOINTMENT,
      'Define E2E_HAS_SOLICITADO_APPOINTMENT=true para consulta com status=SOLICITADO.',
    );

    await goto(ROUTES.dashboardPaciente);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 12_000 });
    await page.getByRole('tab', { name: /próximas/i }).click();

    // SOLICITADO → "Aguardando especialista"
    await expect(page.getByText('Aguardando especialista').first()).toBeVisible({ timeout: 8_000 });
  });

  rdTest('getStatusBadge: "Confirmada" aparece para CONFIRMADO ou confirmed — R4', async ({
    page, goto,
  }) => {
    rdTest.skip(
      !process.env.E2E_HAS_CONFIRMED_APPOINTMENT,
      'Define E2E_HAS_CONFIRMED_APPOINTMENT=true para consulta com status=CONFIRMADO ou confirmed.',
    );
    ...
    // CONFIRMADO ou confirmed → "Confirmada"
    await expect(page.getByText('Confirmada').first()).toBeVisible({ timeout: 8_000 });
  });

  rdTest('estrutura do DashboardPaciente não crasha com aba histórico sem dados', async ({
    page, goto,
  }) => {
    // Sem seed — verifica que a aba funciona mesmo sem dados (estado vazio)
    await goto(ROUTES.dashboardPaciente);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 12_000 });
    ...
    // Ou tem conteúdo ou exibe estado vazio — nunca crasha
    const hasContent = await page.getByText('Concluída').isVisible().catch(() => false);
    const hasEmpty   = await page.getByText('Nenhuma consulta no histórico').isVisible().catch(() => false);
    expect(hasContent || hasEmpty).toBe(true);
  });

  rdTest('estrutura do DashboardPaciente não crasha com aba canceladas sem dados', async ({
    page, goto,
  }) => {
    await goto(ROUTES.dashboardPaciente);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 12_000 });
    ...
    const hasContent = await page.getByText('Cancelada').isVisible().catch(() => false);
    const hasEmpty   = await page.getByText('Nenhuma consulta cancelada').isVisible().catch(() => false);
    expect(hasContent || hasEmpty).toBe(true);
  });
