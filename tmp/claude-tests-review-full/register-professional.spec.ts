...
rdTest.describe('cadastro-profissional — step 99 (sucesso)', () => {
  ...
  rdTest('tela de sucesso exibe h2, subtítulo, aviso amber e dois botões @critical', async ({
    page, goto,
  }) => {
    rdTest.skip(
      !process.env.E2E_NEW_PROFESSIONAL_EMAIL,
      'Define E2E_NEW_PROFESSIONAL_EMAIL — conta criada via cadastro completo que ficou no step 99.',
    );

    await goto(ROUTES.entrar);
    await page.getByLabel('Email').fill(process.env.E2E_NEW_PROFESSIONAL_EMAIL!);
    await page.getByLabel('Senha').fill(process.env.E2E_NEW_PROFESSIONAL_PASSWORD ?? '');
    await page.getByRole('button', { name: 'Entrar' }).click();
    await page.waitForURL(/DashboardProfissional|CadastroProfissional/, { timeout: 20_000 });

    // Se foi para Dashboard, o step 99 foi processado pelo servidor e o usuário
    // está aprovado — step 99 só aparece quando step===99 no estado React
    if (page.url().includes('CadastroProfissional')) {
      await expect(
      ...
      // Aviso amber com próximo passo
      await expect(
        page.getByText('Próximo passo importante')
      ).toBeVisible();
      await expect(
        page.getByText(/complete seus dados.*disponibilidade.*valores/i)
      ).toBeVisible();
